...
{
	int x, y, c;
	auto data = stbi_load(filepath.data(), &x, &y, &c, comp);
	...
	auto name = create_texture_2d(in, ex, x, y, data);
...
GLuint create_texture_cube_from_file(std::array<std::string_view, 6> const& filepath, stb_comp_t comp = STBI_rgb_alpha)
{
	...
	auto name = create_texture_cube(in, ex, x, y, faces);
...
int main(int argc, char* argv[])
{
	...
	auto window = SDL_CreateWindow("\0", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width, window_height, SDL_WINDOW_OPENGL);
	...
	if (auto const glad = gladLoadGL(); glad)
	{
		...
		auto const
			cub_dif = create_texture_2d_from_file(".\\textures\\T_Default_D.png", STBI_rgb),
			cub_spc = create_texture_2d_from_file(".\\textures\\T_Default_S.png", STBI_grey),
			cub_nrm = create_texture_2d_from_file(".\\textures\\T_Default_N.png", STBI_rgb),
			skybox = create_texture_cube_from_file({
			...
			col_tex = create_texture_2d(GL_RGB8, GL_RGB, screen_width, screen_height, nullptr, GL_NEAREST),
			pos_tex = create_texture_2d(GL_RGB16F, GL_RGB, screen_width, screen_height, nullptr, GL_NEAREST),
			nrm_tex = create_texture_2d(GL_RGB16F, GL_RGB, screen_width, screen_height, nullptr, GL_NEAREST),
			alb_tex = create_texture_2d(GL_RGBA16F, GL_RGBA, screen_width, screen_height, nullptr, GL_NEAREST),
			dep_tex = create_texture_2d(GL_DEPTH_COMPONENT32, GL_DEPTH, screen_width, screen_height, nullptr, GL_NEAREST);

		auto const
			buffer_g = create_framebuffer({ pos_tex, nrm_tex, alb_tex }, dep_tex),
			final_col_fb = create_framebuffer({ col_tex });
		...
		while (ev.type != SDL_QUIT)
		{
			...
			glm::vec3 const 
				cam_forward =	camera_rot * glm::vec3(0.0f, 0.0f, 1.0f),
				cam_up		=	camera_rot * glm::vec3(0.0f, 1.0f, 0.0f),
				cam_right	=	camera_rot * glm::vec3(1.0f, 0.0f, 0.0f);
			...
			float const depth_clear_val = 1.0f;
